...
void main() {
  ...
  testWidgets('changed data, error in data, does not save',
      (WidgetTester tester) async {
    MockApi mockApi = MockApi();
    when(mockApi.editAccount(1, 'user@email', null)).thenAnswer(
        (_) async => Future.value({"body": "", "statusCode": "200"}));
    var userApi = {
      "id": 1,
      "username": "user1",
    ...
    await tester.tap(find.byKey(Key('saveAccountButton')));
    await tester.pumpAndSettle();
    expect(find.text("Podaj poprawny adres email"),
        findsOneWidget);
  ...
  testWidgets('changed data, email and telephone exists, does not save',
      (WidgetTester tester) async {
    MockApi mockApi = MockApi();
    when(mockApi.editAccount(1, "user@email.pl", '+48999888777'))
        .thenAnswer((_) async => Future.value({
              "body":
  ...
  testWidgets('english, body is empty, does not save', (WidgetTester tester) async {
    MockApi mockApi = MockApi();
    MockSecureStorage mockSecureStorage = MockSecureStorage();
    Account user = Account(
        id: 1,
  ...
  testWidgets('english superuser, changed email, saves', (WidgetTester tester) async {
    MockApi mockApi = MockApi();
    when(mockApi.editAccount(1, 'user@email.pl', null)).thenAnswer(
            (_) async => Future.value({"body": "", "statusCode": "200"}));
    var userApi = {
  ...
  testWidgets('english changed data, error in data, does not save',
        ...
        await tester.tap(find.byKey(Key('saveAccountButton')));
        await tester.pumpAndSettle();
        expect(find.text("Enter a valid email address"),
            findsOneWidget);
        verifyNever(await mockApi.editAccount(1, 'user@email', null));
      });
  ...
  testWidgets('english changed data, email and telephone invalid, does not save',
          (WidgetTester tester) async {
        MockApi mockApi = MockApi();
        when(mockApi.editAccount(1, "user@email.pl", '+48999888777'))
            .thenAnswer((_) async => Future.value({
